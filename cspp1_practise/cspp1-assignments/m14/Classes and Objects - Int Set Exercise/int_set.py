...
# Hint: look through the Python docs to figure out what you'll need to solve this problem.
    def intersect(self, other):
        new_set = set()
        for lo_op in self.vals:
            if lo_op in other.vals:
                new_set.add(lo_op)
        if len(new_set) == 0:
            return '{}', len(new_set)
        return new_set.__str__(), len(new_set)
    def __len__(self):
        return len(self)
...
def main():
    ...
    for j in l2:
        setB.insert(int(j))
    (val, le_n) = setA.intersect(setB)
    print('Intersection set is ', val)
    print('Length of Intersection set is ', le_n)
